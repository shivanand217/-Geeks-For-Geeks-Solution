...
/** we just check the lower limit of that number of digits and upper limit then we check for each pairs between them thats it **/
...
int lower_lim = 0;
if(lower_lim == 0){lower_lim =10;tmp--;}
...
//cout<<lower_lim<<" "<<upper_lim<<endl;

LL int tmp1;
LL int ans = -1LL;

for(int i=lower_lim; i<upper_lim/2; i++)
{
    for(int j=lower_lim+1; i<=upper_lim/2; i++)
    {
        tmp = ((i)*(j));

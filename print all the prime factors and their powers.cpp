...
/** we are using segmented seive for generating all the primes less than n then we calculate the answer **/
...
void calculate_power_of_primes(vector<pair<int,int> > &power_of_primes, int n)
{
    int tmp = n;int k;
    ...
    for(int i=0; i<prime.size(); i++)
    {
        ...
        if(tmp % p == 0)
        {
            ...
            power_of_primes.push_back(make_pair(p,k));
        }
    }
}
...
vector<pair<int,int> > powers_of_primes;

calculate_power_of_primes(powers_of_primes, n);

for(int i=0; i<powers_of_primes.size(); i++)
{
    cout<<powers_of_primes[i].first<<" "<<powers_of_primes[i].second<<endl;
}

...
// create a prefix sum array of the primes
int prefix_prime[primes.size()];

prefix_prime[0] = primes[0];
for(int i=1; i<primes.size(); i++)
{
    prefix_prime[i] = prefix_prime[i-1] + primes[i];
}
...
for(int i=0; i < primes.size(); i++)
{
    for(int j=0; j <= i; j++)
    {
        ...
        int val = prefix_prime[i] - prefix_prime[j];

        if(binary_search(primes.begin(), primes.end(), val))
        {
            if(max_ < len)
                {max_ = len;prime_nu = val;}
        }
    }
}

// cout the maximum prime
cout<<max_<<" -> "<<prime_nu<<endl;

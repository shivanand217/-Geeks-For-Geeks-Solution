...
/** finding the nth s-gonal number like 2nd triangular number , 4th pentagonal number etc.
    is so simple is that there is a formula for that
    p(s,n) = ((n^2) *(s-2) - n * (s-4)) /4
    by this formula we can find any **/
...
int nth_s_gonal_number(int n,int s)
{

    int ans = ((n*n)*(s-2) - n*(s-4))/2;
...
int ans = nth_s_gonal_number(n,s);

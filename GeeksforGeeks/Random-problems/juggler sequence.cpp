...
/** juggler sequence is given by the following
             {
    a(k+1) =   floor(sqrt(a(k))) for even a(k)
               floor(a(k) * sqrt(a(k))) for odd a(k)
             }
**/
...
void juggler_sequence(int n) // start of sequence with n
{
    int prev = n;int k;
